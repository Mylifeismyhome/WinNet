...
{
	NET_RSA rsa;
	setPublicKey(rsa.CreatePublicKey(size, 3));
	setPrivateKey(rsa.CreatePrivateKey(size, 3));

	setPublicKeySize(size);
	setPrivateKeySize(size);
...
void  Server::cryption_t::setPublicKeySize(const size_t size)
{
	_RSAPublicKeySize = size;
}

void Server::cryption_t::setPrivateKeySize(const size_t size)
{
	_RSAPrivateKeySize = size;
}

size_t Server::cryption_t::getPublicKeySize()  const
{
	return _RSAPublicKeySize;
}

size_t Server::cryption_t::getPrivateKeySize() const
{
	return _RSAPrivateKeySize;
}
#pragma endregion
...
Server::NET_IPEER Server::InsertPeer(const sockaddr_in client_addr, const SOCKET socket)
{
	...
	LOG_PEER(CSTRING("[%s] - Peer ('%s'): connected!"), GetServerName(), newPeer.getIPAddr());
	return newPeer;
}
...
bool Server::ErasePeer(NET_PEER peer)
{
	if (!peer.isAsync)
	{
		...
		LOG_PEER(CSTRING("[%s] - Peer ('%s'): disconnected!"), GetServerName(), peer.getIPAddr());
...
const char* Server::NET_IPEER::getIPAddr() const
{
...
void Server::DoSend(NET_PEER peer, const int id, NET_PACKAGE pkg)
{
	...
	if (GetCryptPackage() && peer.cryption.getHandshakeStatus())
	{
		...
		NET_RSA rsa;
		if (!rsa.Init(peer.cryption.getPublicKey(), (char*)CSTRING("")))
		{
			Key.free();
			IV.free();
...
short Server::Handshake(NET_PEER peer)
{
	...
	LOG_PEER(CSTRING("[%s] - Peer ('%s'): Something bad happen on Handshake"), GetServerName(), peer.getIPAddr());
...
void Server::ReceiveThread(const sockaddr_in client_addr, const SOCKET socket)
{
	...
	do
	{
		...
		{
			LOG_PEER(CSTRING("[%s] - Peer ('%s'): dropped due to websocket!"), GetServerName(), peer.getIPAddr());
...
void Server::ExecutePackage(NET_PEER peer, const size_t size, const size_t begin)
{
	...
	if (GetCryptPackage() && peer.cryption.getHandshakeStatus())
	{
		...
		// Init RSA
		NET_RSA rsa;
		//if (!rsa.Init((char*)CSTRING(""), peer.cryption.RSAPrivateKey.get()))
		if (!rsa.Init((char*)CSTRING(""), peer.cryption.getPrivateKey()))
		{
			AESKey.free();
			AESIV.free();
...
/* Handshake is used to block connections from web-sockets */
bool Server::Handshake(SOCKET curSocket) const
{
	CPOINTER<char> temp_network_data(ALLOC<char>(DEFAULT_SERVER_MAX_PACKET_SIZE + 1));
	const auto data_size = recv(curSocket, temp_network_data.get(), DEFAULT_SERVER_MAX_PACKET_SIZE, 0);
	...
	std::string CheckNetworkData(temp_network_data.get());
	const auto pos = CheckNetworkData.find(CSTRING("GET / HTTP/1.1"));
	if (pos != std::string::npos)
	{
		std::map<std::string, std::string> entries;

		std::istringstream s(temp_network_data.get());
		std::string header;
		std::string::size_type end;
		...
		while (std::getline(s, header) && header != CSTRING("\r"))
		{
			...
			if (end != std::string::npos)
			{
				auto key = header.substr(0, end);
				auto val = header.substr(end + 2);
		...
		sha.Reset();
		sha << entries["Sec-WebSocket-Key"].c_str();
		...
		// Encode Base64
		size_t outlen = 0;
		auto enc_Sec_Key = Base64_Encode(reinterpret_cast<BYTE*>(message_digest), 20, &outlen);

		// Create Response
		CPOINTER<char> buffer(ALLOC<char>(DEFAULT_SERVER_MAX_PACKET_SIZE));
		sprintf(buffer.get(), CSTRING("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: %s\r\n\r\n"), reinterpret_cast<char*>(enc_Sec_Key));

		FREE(enc_Sec_Key);

		const auto res = send(curSocket, buffer.get(), static_cast<int>(strlen(buffer.get())), 0);
		if (res == SOCKET_ERROR)
			LOG_ERROR(CSTRING("[%s] - Handshake response failed!"), GetServerName());

		temp_network_data.free();
		return true;
	}
