...
#undef SHA1_ROUND_4
		...
		const SHA1& SHA1::to_hex(char* hex, bool zero_terminate, const char* alphabet)
		{
			int k = 0;
			for (int i = 0; i < 5; i++) {
				for (int j = 7; j >= 0; j--) {
		...
		const SHA1& SHA1::to_base64(char* base64, bool zero_terminate)
		{
			...
			base64[NET_SHA1_BASE64_SIZE - 2] = '=';
			if (zero_terminate) base64[NET_SHA1_BASE64_SIZE - 1] = '\0';
			return *this;
		}
	}
}